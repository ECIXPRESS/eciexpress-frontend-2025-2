...
export const ProductoCard: React.FC<ProductoCardProps> = ({ 
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-col flex-grow p-4">
        ...
        <div className="flex gap-2 mt-auto">
          <button 
            onClick={handleFlip}
            className="flex-1 bg-white hover:bg-gray-50 text-yellow-500 border-2 border-yellow-400 py-2.5 px-3 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center justify-center"
          >
            Ver Detalles
          </button>
          <button 
            onClick={() => onEditStock(producto.id)}
            className="flex-1 bg-yellow-400 hover:bg-yellow-500 text-white py-2.5 px-3 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center justify-center shadow-md hover:shadow-lg"
          >
  ...
  const CardBack = () => (
    <div className="h-full flex flex-col">
      ...
      <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white p-4 rounded-t-xl">
        <div className="flex justify-between items-start mb-2">
          <div className="flex-1 min-w-0 mr-2">
            <h3 className="font-bold text-lg truncate">{producto.nombre}</h3>
            <p className="text-sm text-yellow-100">{producto.categoria}</p>
          </div>
          <button 
            onClick={handleFlip}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/20">
          <span className="text-sm text-yellow-100">Precio</span>
      ...
      <div className="flex-grow overflow-y-auto p-4">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-xs font-semibold text-yellow-800 mb-1">Última actualización</p>
      ...
      <div className="p-4 border-t border-gray-100">
        <button 
          onClick={() => onEditStock(producto.id)}
          className="w-full bg-yellow-400 hover:bg-yellow-500 text-white py-3 px-4 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center shadow-md hover:shadow-lg"
        >
