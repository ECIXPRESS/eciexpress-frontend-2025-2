...
import {
  mockDailySalesReport,
  mockWeeklySalesReport,
  mockMonthlySalesReport,
  mockSummaryReport,
  mockTopProducts
...
class StatisticsService {
   ...
   */
  async getDaily(date: string, store: string): Promise<DailySalesReport> {
   ...
   */
  async getWeekly(date: string, store: string): Promise<WeeklySalesReport> {
    await this.delay(300);
   ...
   */
  async getMonthly(year: number, month: number, store: string): Promise<MonthlySalesReport> {
    await this.delay(300);
   ...
   */
  async getSummary(store: string): Promise<SummaryReport> {
    await this.delay(300);
   ...
   */
  async getTopProducts(store: string): Promise<ProductSalesReport[]> {
    await this.delay(300);
   ...
   */
  async exportAllStatistics(store: string, format: 'pdf' | 'excel'): Promise<void> {
